...
import { ArrowLeft, Edit, Users, Calendar, Heart, Download, TreePine } from 'lucide-react';
...
const DogProfile: React.FC = () => {
  ...
  const handleLinkParents = async () => {
    try {
      ...
      if (response.success) {
        toast.success('Ouders succesvol gekoppeld!');
        loadDog(); // Reload dog data
      } else {
        toast.error(response.error || 'Fout bij het koppelen van ouders');
      }
    } catch (error) {
      console.error('Error linking parents:', error);
      toast.error('Fout bij het koppelen van ouders');
    } finally {
  ...
  const handleGeneratePedigree = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        toast.error(response.error || 'Fout bij het genereren van stamboom');
      }
    } catch (error) {
      console.error('Error generating pedigree:', error);
      toast.error('Fout bij het genereren van stamboom');
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-lg text-gray-900">
                      {dog.gender === 'male' ? 'Mannetje' : 'Vrouwtje'}
                    </p>
                  </div>
                  <div>
                    <label className="text-sm font-medium text-gray-600">Leeftijd</label>
                    <p className="text-lg text-gray-900">{getAge(dog.birth_date)} jaar</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="text-sm font-medium text-gray-600">Ras</label>
          ...
          {/* Parents */}
          <div className="card p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Ouders</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Father */}
              <div>
                <label className="text-sm font-medium text-gray-600 mb-2 block">Vader</label>
                <select
                  value={parentData.father_id}
                  onChange={(e) => setParentData(prev => ({ ...prev, father_id: e.target.value }))}
                  className="input"
                >
                  <option value="">Geen vader geselecteerd</option>
                  {availableDogs
              ...
              {/* Mother */}
              <div>
                <label className="text-sm font-medium text-gray-600 mb-2 block">Moeder</label>
                <select
                  value={parentData.mother_id}
                  onChange={(e) => setParentData(prev => ({ ...prev, mother_id: e.target.value }))}
                  className="input"
                >
                  <option value="">Geen moeder geselecteerd</option>
                  {availableDogs
            ...
            <div className="mt-4">
              ...
              >
                {linkingParents ? 'Koppelen...' : 'Ouders Koppelen'}
              </button>
            </div>
          </div>
        </div>
